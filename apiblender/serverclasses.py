...
class Policy:
    ...
    def __init__(self, policy_config):
        ...
        self.too_many_calls_response_status = policy_config["too_many_calls_response_status"]
        self.too_many_calls_waiting_seconds = policy_config["too_many_calls_waiting_seconds"]
...
class Request:
    ...
    def set_url_param(self, url_param_to_set):
        ...
        url_param_config = [url_param_to_set[0], None, None, url_param_to_set[1]]
    ...
    def get_total_url_params(self):
        total_url_params = {}
        for url_param in self.url_params:
            if url_param.value is not None:
                total_url_params.update({url_param.key: url_param.value})
            elif not url_param.optional:
                new_value = raw_input('{} is required, please input its value: '.format(url_param.key))
